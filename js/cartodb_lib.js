...
var CartoDbLib = {
  ...
  initialize: function(){
	...
	//})//.addTo(CartoDbLib.map);
	  ...
	  CartoDbLib.basemap = L.tileLayer('http://www.google.co.in/maps/vt/lyrs=r&hl=en&x={x}&y={y}&z={z}&s=Ga', {
		detectRetina: true,
		  maxNativeZoom: 25,
		  maxZoom: 30,
	  }).addTo(CartoDbLib.map);
      ...
      CartoDbLib.info.update = function (props) {
			...
			this._div.innerHTML = "<div class='legendIcon "+props.plot_spe_1+"'></div>  <b>| Plot-type: </b>"+props.plot_gener +"<b> | Plot-no: </b>" + props.plot_no+"<b> | Plot-Extent: </b>"+props.plot_area_+"<b> | Plot-Face: </b>"+facing+"<b> | Location: </b>"+props.plot_allot+"<b> | Recommended Price:  </b> <i class='fa fa-inr' aria-hidden='true'></i>"+props.price_min+"-"+props.price_max+" /Per Sq. Yd";
        $(this._div).show();
      };
	...
	CartoDbLib.info.addTo(CartoDbLib.map);
      ...
      CartoDbLib.cinfo.update = function (props) {
			this._div.innerHTML = "<div class='legendIcon "+props.plot_spe_1+"'></div><b>  | Landuse-type: </b>"+props.plot_gener +"<b> | Landuse-details: </b>"+props.plot_speci;
        $(this._div).show();
      };
      ...
      cartodb.createLayer(CartoDbLib.map, CartoDbLib.layerUrl )
        ...
        .done(function(layer) {
          ...
          var sublayer = layer.getSubLayer(0);
			...
			sublayer.setInteractivity('cartodb_id,plot_no,plot_speci,plot_spe_1,plot_gener,plot_area_,plot_allot,northfacin,southfacin,eastfacing,westfacing,price_min');
			sublayer.on('featureOver', function(e, latlng, pos, data, subLayerIndex) {
            $('#mapCanvas div').css('cursor','pointer');
			...
			sublayer.on('featureClick', function(e, latlng, pos, data){
            CartoDbLib.getOneZone(data['cartodb_id'], latlng, "onlylanduse");
			});
			sublayer.on('featureOut', function(e, latlng, pos, data, subLayerIndex) {
  ...
  drawLayerControl: function() {
    if(
      ...
      }, { collapsed: true, autoZIndex: true }).addTo(CartoDbLib.map);
    }
  },
  ...
  getOneZone: function(cartodb_id, click_latlng, table){
    ...
    .done(function(data){
	  ...
	  var popup_content = 
		...
		<div id='cus-info-body-container'><b>Plot No: </b>"+props.plot_no+"<br><b>Township: </b>"+props.plot_allot+"<br><b>Plot Extent: </b>"+props.plot_area_+" Sq. Yd<br><b>Property Type: <div class='legendIcon "+props.plot_spe_1+"'></div></b>"+" "+props.plot_gener +"<br></div>\
        <div id='cus-info-details'><div id='cus-info-details-header'><b style='color:#fff'>Sector Price Details</b></div><b>Recommended Price:  </b> <i class='fa fa-inr' aria-hidden='true'></i>"+props.price_min+"-"+props.price_max+" /Per Sq. Yd<br><b>Market Price: </b>"+marketprice +"\
        <div id='cus-info-details'><div id='cus-info-details-header'><b style='color:#fff'>Construction Details</b></div><b>Maximum Built-up Area: </b>"+props.maximu_bui+" Sq. yd <br><b>Total Floor Area: </b>"+props.total_floo+" Sq. ft<br><b>No Of Floors Allowed: </b>"+props.no_of_floo+"</div></div>\
		<div class='cus-info-button-group'><div id='cus-info-button-lets-talk' class='cus-info-button' onclick='imIntrested("+props.plot_no+");'>I am interested</div></div>\
       	";
	  }
	  else {
	  var popup_content = 
